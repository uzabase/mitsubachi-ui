...
import { type MiUzabaseLogo } from "../../src/components/logo";
...
const meta = {
  ...
  tags: ["!dev-only"],
} satisfies Meta<MiUzabaseLogo>;

export default meta;
type Story = StoryObj<MiUzabaseLogo>;

export const Default: Story = {
  render: ({ inverse }) => {
    return html`<mi-uzabase-logo ?inverse=${inverse}></mi-uzabase-logo>`;
  },
};
  ...
  render: () => {
    return html`
      <div style="background: #333; padding: 24px; border-radius: 8px;">
        <mi-uzabase-logo inverse></mi-uzabase-logo>
      </div>
    `;
  },
};
          ...
          <tbody>
            <tr style="height: 4rem;">
              <td style="padding: 12px 16px; border-top: 1px solid #e0e0e0;">
                <mi-uzabase-logo></mi-uzabase-logo>
              </td>
              <td
