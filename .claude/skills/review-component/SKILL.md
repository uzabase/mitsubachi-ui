...
イベント関連は **必ず `docs/event-architecture.md` を読み**、以下の観点でチェックする:

- **bubbles / composed が false か**: `bubbles: true` や `composed: true` を grep で検出し、`false` でない箇所を洗い出す
- **カスタムイベントを定義していないか**: `new CustomEvent` を grep で検出する。定義されている場合、対応するネイティブHTMLイベントが本当に存在しないか確認する
- **イベント名にプレフィックスを付けていないか**: イベント名はネイティブHTMLの命名規則に従う（`mi-` 等のプレフィックスは付けない）
- **Shadow DOM 内イベントの再発火**: Shadow DOM 内で発火したイベントはルート要素で受け取り、ルート要素自身から発火し直しているか
...
- **ネイティブ互換・内部イベント漏洩防止・querySelector の安全性**: `docs/event-architecture.md` の該当セクションを参照
...
- イベントの `bubbles` / `composed` が `false` であることを検証するテストがあるか
