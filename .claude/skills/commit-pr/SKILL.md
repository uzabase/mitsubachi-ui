...
- `docs/commit-push-pr.md` の PR本文フォーマットに従って本文を構成する。
  ...
  - **レビュワーが最初に読む分量を短く保つ。** 判断を仰ぎたい点を上に置き、背景は `<details>` に畳む。
  - **公開する API（属性・slot・既定値・役割）は表にする。**
  - **テストと Story の内容は列挙しない**（「内容はコミットをご覧ください」で済ませる）。diff で分かる情報を書くと、判断してほしい箇所が埋もれる。
  - 「レビューで見てほしいところ」は**判断を仰ぎたい点が無ければ節ごと省略する**。件数に上限は設けず、**判断に必要な情報を簡潔にまとめる**ことを優先する（1件あたりは「何を選んだか」「なぜか」で足り、経緯は `<details>` へ）。「後から変えづらいもの」（公開 API・依存の追加・DOM 構造・デザインとの意図的な差分）を優先して並べる。
