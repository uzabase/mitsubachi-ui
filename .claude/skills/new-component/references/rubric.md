...
観点の出典は `docs/` であり、ここに勝手な観点を足さない。docs が変わったらこの表を更新する。

## このルーブリックの読み方（Anthropic 推奨形式）

- **1項目=1観点（atomic）**。複合質問にしない。
- **YES/NO で客観判定できる断定文**で書く（「適切か？」ではなく「〜している」）。
- **PASS条件**を明文化する。曖昧語（適切・良い）で逃げない。
- **重要度は項目ごとに固定**: 高(critical) / 中(major) / 低(minor)。
- **確認**列: `hook` = 決定的検証（typecheck/lint/test）で機械判定済み → 結果をそのまま正とし再評価しない。`read` = コードを読んで主観判定する。
- **適用**列: 常時／条件付き。条件に当たらなければ `n/a`。
