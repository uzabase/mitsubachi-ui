---
name: new-component-v2
description: >
  新しい mi- Web Component を実装→評価の自動ループで作成する（ルーブリック評価版）。
  評価は docs/ から起こした固定ルーブリック（references/rubric.md）を ID 単位で採点する点が
  new-component と異なる。Figma と（あれば）React 版を参照元に docs の各規約に従う。
  別アプローチの試行として new-component と共存する。
...
hooks:
  SubagentStop:
    - hooks:
        - type: command
          command: "bash .claude/skills/new-component-v2/scripts/run-checks.sh"
---
...
- **境界**: 実装手順は `.claude/skills/new-component-v2/references/generator.md` を読んで従う。`docs/` の規約に従う。**コミットはしない**。中/低 finding は任意対応。スコープ外のリファクタはしない。
...
- **境界**: 評価手順は `.claude/skills/new-component-v2/references/evaluator.md` を読んで従う。**評価のみ。コードは修正しない。**
