...
`new-component-v2`（Orchestrator）から Evaluator サブエージェントとして起動されたときに読む評価手順。
実装↔評価ループの評価担当。
...
3. **必ず `.claude/skills/new-component-v2/references/rubric.md` を読み、その全項目（A〜M の各 ID）を1つずつ採点する。**
